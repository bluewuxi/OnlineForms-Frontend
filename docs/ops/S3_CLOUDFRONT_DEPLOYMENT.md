...
- CI fallback hydration:
  ...
  - if `VITE_COGNITO_DOMAIN` is still empty, CI reads backend stack output `CognitoHostedUiDomainUrl`.
  - if `CognitoHostedUiDomainUrl` is absent, CI falls back to `CognitoUserPoolId` + `cognito-idp describe-user-pool`.
